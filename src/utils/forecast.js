...
const forecast = (latitude, longitude, callback) => {
    ...
    request({url, json: true}, (error, { body }) => {
        if(error){
            ...
            callback(undefined, body.current.weather_descriptions[0] + ', There is a ' + body.current.precip + '% chance of rain. The current temprature ' 
                        + body.current.temperature + ' degress out, but feels like ' + body.current.feelslike + ' degrees.')
            //{
